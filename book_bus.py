...
mydb = mysql.connector.connect(host="localhost", user="root", passwd="Jagdish@1", database='project')
...
class Ride(): 
    ...
    def login(self):
        ...
        mycursor.execute(f"insert into user_detail1(id, name, age, email, number, reg_time) values('{self.id_1}', '{self.name}', {self.age_1}, '{self.email}', '{self.number}', '{self.time}');")
        mydb.commit()
